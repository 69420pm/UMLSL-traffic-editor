...
class TestSaveTrafficSnapshot(unittest.TestCase):
    ...
    def test_execute_serializes_and_writes_file(self):
        ...
        try:
            with patch(
                "pse.umlsl_editor.src.commands.persistence.save_traffic_snapshot.PersistenceService.serialize",
                autospec=True,
            ) as serialize:
...
class TestSaveAsTrafficSnapshot(unittest.TestCase):
    ...
    def test_execute_serializes_and_writes_file(self):
        ...
        try:
            with patch(
                "pse.umlsl_editor.src.commands.persistence.save_as_traffic_snapshot.PersistenceService.serialize",
                autospec=True,
            ) as serialize:
                serialize.return_value = payload
...
class TestLoadTrafficSnapshot(unittest.TestCase):
    ...
    def test_execute_deserializes_and_replaces_snapshot(self):
        ...
        try:
            with patch(
                "pse.umlsl_editor.src.commands.persistence.load_traffic_snapshot.PersistenceService.deserialize",
                autospec=True,
            ) as deserialize:
                cmd = LoadTrafficSnapshot(file_path=tmp_path, application_controller=controller)
                cmd.execute()
